...
const parseJsonField = (req, field) => {
  ...
  if (typeof val === 'string') {
    try {
      ...
      req.body[field] = val.split(',').filter(Boolean);
    }
  }
...
const parseRestaurantFormData = (req, res, next) => {
  parseJsonField(req, 'operating_days');
  parseJsonField(req, 'payment_methods');
  parseJsonField(req, 'special_features');
