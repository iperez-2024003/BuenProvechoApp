...
export const redeemPoints = asyncHandler(async (req, res) => {
  try {
    ...
    if (!Number.isInteger(cost) || cost <= 0) {
      return res.status(400).json({
        success: false,
        message: 'El costo debe ser un número entero positivo',
      });
    }
    ...
    const currentPoints = typeof user.Points === 'number' ? user.Points : parseInt(user.Points, 10) || 0;
...
export const addPoints = asyncHandler(async (req, res) => {
  try {
    ...
    if (!Number.isInteger(amount) || amount <= 0) {
      return res.status(400).json({
        success: false,
        message: 'El monto debe ser un número entero positivo',
      });
    }
